...
#define MCUISR mcuISR
#define STACK_TOP_ADDRESS   0x20008000
...
#define CONTROL_R           (*((volatile unsigned long *) 0xE000ED14))


/*
    - We will not move the vector table from FLASH to RAM so, there will be no offset from base address
    - 0x00000000
 */
#define VTABLE_OFFSET_VAL 0x00000000
...
void Reset_Handler(void) {
    ...
    /* MCU INITIALIZATION */
    /* Vector table offset from base address 0x00000000*/
    NVIC_VTABLE_R = VTABLE_OFFSET_VAL ;

    /* disable watchdog timer clock */
    SYSCTL_RCGCWD_R &= ~(0b11) ;

    /* initialize sysclock to PIOCS and MOSC,PLL off and crystal to 16 MHz , sysdiv for sysclock and pwm is off*/
    SYSCTL_RCC_R &= ~(0b00001000010100000000000000100000) ;
    SYSCTL_RCC_R |=  (0b00000000000000000010100000010001) ;

    NVIC_CFG_CTRL_R &= (0b1 << 9 ) ; /* Stack Alignment on Exception Entry  is 4-byte aligned */
